...
import type { CatAvailability, CatProfile } from '../../types/models'

const statuses: Array<CatAvailability | 'All'> = ['All', 'At the cafe', 'Resting', 'In care']

export function CatsPage() {
  const [items, setItems] = useState<CatProfile[]>([])
  const [status, setStatus] = useState<CatAvailability | 'All'>('All')

  useEffect(() => { catalogRepository.listCats().then(setItems) }, [])
  ...
  return (
    <section className="content-page page-width">
      <div className="playful-hero">
        ...
        <div className="playful-hero__stamp"><MdFavorite /><strong>12</strong><span>cats call Loaf home</span></div>
      </div>
      ...
      <div className="cat-grid">
        {visible.map((cat) => (
          <article className="cat-card" key={cat.id}>
            ...
            <div className="cat-card__body"><div><h3>{cat.name}</h3><span>{cat.breed} · {cat.age} years</span></div><p>{cat.description}</p><div className="cat-traits"><span><MdFavorite />Friendly {cat.friendliness}/5</span><span><MdSchedule />Playful {cat.playfulness}/5</span></div></div>
          </article>
        ))}
      </div>
    </section>
